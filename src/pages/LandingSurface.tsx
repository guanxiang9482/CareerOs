...
export function LandingSurface({ onExplore, onEmployerSignup, onSelectRole, stage, onNavigateToHavenChat }: LandingSurfaceProps) {
  ...
  // 1. DYNAMIC & DEDUPLICATED LOGIC
  // Extracts unique companies and computes real job numbers
  const dynamicCompanies = useMemo(() => {
    const uniqueNames = new Set<string>()
    const uniqueCompanies: typeof COMPANIES = []

    // Eliminate any random duplications generated by the PRNG
    ...
    }).filter(c => c.roles > 0).sort((a, b) => b.roles - a.roles) // Rank by most jobs available
  }, [])

  // 2. LIVE GENERATED HIGHLIGHT PANELS
  // Maps the top generated companies into the Spotlight and Fast Growth components
  const spotlightCompanies = useMemo(() => {
    if (dynamicCompanies.length === 0) return []
    return dynamicCompanies.slice(0, 5).map((c, i) => ({
      ...c,
      tag: c.industry.toUpperCase(),
      blurb: 'First-class business, first-class way.',
      desc: `Global operations pushing boundaries in ${c.industry}. Ranked high for rapid trajectory jumps and sustainable ecosystem scaling.`,
      employees: `${50 + i * 15}k+`,
      hubs: `${12 + i * 3}+`,
  ...
  const fastGrowthCompanies = useMemo(() => {
    if (dynamicCompanies.length < 6) return spotlightCompanies
    return dynamicCompanies.slice(5, 10).map((c, i) => ({
      ...c,
      tag: c.industry.toUpperCase(),
      blurb: 'High Promotion Velocity Framework.',
      desc: `Expanding unified ${c.industry.toLowerCase()} data grids by +45% this fiscal quarter window. Offers accelerated pathways into high-autonomy technical lead roles.`,
      employees: `${8 + i * 2}k+`,
      hubs: `${4 + i}+`,
      rating: (4.5 + (i % 4) * 0.1).toFixed(1)
    }))
  }, [dynamicCompanies, spotlightCompanies])

  useEffect(() => {
    if (subView !== 'HOME' || spotlightCompanies.length === 0) return
    const interval = setInterval(() => {
      setSpotlightIdx((prev) => (prev + 1) % spotlightCompanies.length)
    }, 5000)
    return () => clearInterval(interval)
  }, [subView, spotlightCompanies.length])

  const selectedCompanyProfile = useMemo<CompanyProfile>(() => {
    const found = dynamicCompanies.find(c => c.name.toLowerCase() === selectedCompanyName.toLowerCase())
    return {
      name: found?.name || selectedCompanyName,
      industry: found?.industry || 'Technology',
      location: found?.location || 'Kuala Lumpur',
      rating: '4.7',
      openRoles: found?.roles || 0
    }
  }, [selectedCompanyName, dynamicCompanies])

  const companyRoles = useMemo(() => {
    return JOBS.filter(j => j.company.toLowerCase() === selectedCompanyName.toLowerCase())
  }, [selectedCompanyName])
    ...
    return (
      <div className="mx-auto max-w-[1000px] px-6 py-12 bg-[#FDFBF9]">
        ...
        <div className="rounded-xl border border-[#EBE7E0] bg-white p-8 shadow-xs">
          ...
          <h3 className="text-sm font-mono uppercase tracking-wider text-[#9A7B56] mt-8 mb-4">Open Positions Pipeline ({selectedCompanyProfile.openRoles})</h3>
  ...
  const currentSpotlight = spotlightCompanies[spotlightIdx] || spotlightCompanies[0]
  const currentFastGrowth = fastGrowthCompanies[0]
  ...
  return (
    <div className="bg-[#FDFBF9]">
      ...
      {currentSpotlight && currentFastGrowth && (
        <section className="mx-auto max-w-[1100px] px-6 py-20 grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="rounded-xl border border-[#9A7B56]/40 bg-white shadow-md relative overflow-hidden flex flex-col justify-between min-h-[360px]">
              ...
              <div className="p-6 flex-1 flex flex-col justify-between">
                ...
                <div className="border-t border-[#F1EDE5] pt-4 mt-6 grid grid-cols-3 gap-2 text-center bg-[#FAF8F5] rounded-lg p-3">
                  ...
                  <div>
                    <p className="font-mono text-base font-bold text-emerald-700">{currentSpotlight.roles} active</p>
          ...
          <div className="lg:col-span-6 space-y-4">
            ...
            <div className="rounded-xl border border-[#9A7B56]/40 bg-white shadow-md relative overflow-hidden flex flex-col justify-between min-h-[360px]">
              <div className="bg-[#0B1E33] p-4 text-white flex justify-between items-center">
                <span className="text-[10px] font-mono uppercase tracking-widest text-[#B5A88F]">
                  ✦ {currentFastGrowth.tag}
                </span>
                <span className="bg-white/10 text-white font-mono text-xs px-2.5 py-0.5 rounded-full border border-white/10">
                  Score Rating: ★ {currentFastGrowth.rating}
                </span>
              </div>

              <div className="p-6 flex-1 flex flex-col justify-between">
                <div>
                  <h4 className="text-2xl font-bold text-[#0B1E33] tracking-tight">{currentFastGrowth.name}</h4>
                  <p className="font-serif italic font-normal text-[#9A7B56] text-sm mt-0.5">"{currentFastGrowth.blurb}"</p>
                  <p className="text-xs text-[#6B5A44] leading-relaxed mt-3.5">{currentFastGrowth.desc}</p>
                </div>

                <div className="border-t border-[#F1EDE5] pt-4 mt-6 grid grid-cols-3 gap-2 text-center bg-[#FAF8F5] rounded-lg p-3">
                  <div>
                    <p className="font-mono text-base font-bold text-[#0B1E33]">{currentFastGrowth.employees}</p>
                    <p className="text-[9px] font-mono uppercase tracking-wider text-[#9A7B56] mt-0.5">Ecosystem Staff</p>
                  </div>
                  <div>
                    <p className="font-mono text-base font-bold text-[#0B1E33]">{currentFastGrowth.hubs}</p>
                    <p className="text-[9px] font-mono uppercase tracking-wider text-[#9A7B56] mt-0.5">Global Hubs</p>
                  </div>
                  <div>
                    <p className="font-mono text-base font-bold text-emerald-700">{currentFastGrowth.roles} active</p>
                    <p className="text-[9px] font-mono uppercase tracking-wider text-emerald-700 mt-0.5">Live Roles</p>
                  </div>
                </div>
                ...
                <div className="mt-5 flex gap-3">
                  <button 
                    onClick={() => { setSelectedCompanyName(currentFastGrowth.name); setSubView('PROFILE') }}
